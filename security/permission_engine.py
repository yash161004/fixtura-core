...
def check(capability_token: CapabilityToken, tool_name: str, arguments: Dict[str, Any]) -> Tuple[bool, str]:
    """
    ...
    if tool_name == "filesystem_tool":
        ...
        if not path:
            return False, "Filesystem path missing."
        ...
        if operation == "read":
            ...
            return False, f"Unknown filesystem operation: {operation}"
            
    elif tool_name == "sqlite_tool":
        operation = arguments.get("operation")
        if operation == "read":
            if not capability_token.db.read:
                return False, "Database read access denied."
            return True, ""
        elif operation == "write":
            if not capability_token.db.write:
                return False, "Database write access denied."
            return True, ""
        else:
        ...
        if domain not in capability_token.http.allowed_domains:
            return False, f"Domain not in allow-list: {domain}"
